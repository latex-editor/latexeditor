...
function renderText(text){
  ...
  try {
    if (window.latexjs && window.latexjs.parse) {
      ...
      const doc = latexjs.parse(s, { generator: generator });
      const div = document.createElement('div');
      div.appendChild(doc.domFragment());
      return div.innerHTML;
    }
  } catch (err) {
    // LaTeX 파싱 에러 발생 시 입력 중인 텍스트가 깨지지 않도록 안전 처리
  }

  // Fallback rendering
  let formatted = escapeHTML(s).replace(/\\\\/g, '\n');
  return formatted.replace(/`([^`]+)`/g, '<code>$1</code>').replace(/\n/g, '<br>');
}
...
document.getElementById('addExample').addEventListener('click',()=>{data.examples.push({input:'',output:''});renderEditor();renderPreview();saveDraft()});
...
document.getElementById('saveBtn').addEventListener('click',()=>{
  ...
  if (saves[key]) {
    if (!confirm(`"${key}" 이름으로 저장된 데이터가 이미 있습니다. 덮어씌우시겠습니까?`)) return;
  }
...
document.getElementById('loadBtn').addEventListener('click',()=>{
  ...
  if (!isNaN(num) && num >= 1 && num <= keys.length && String(num) === keyInput) {
...
document.getElementById('fileInput').addEventListener('change',async e=>{const f=e.target.files[0];if(!f)return;try{const x=JSON.parse(await f.text());if(!x||typeof x!=='object'||!Array.isArray(x.examples))throw new Error();data={...structuredClone(initial),...x};saveDraft();renderEditor();renderPreview()}catch{alert('올바른 Polygon Editor JSON 파일이 아닙니다.')}e.target.value='' });
document.getElementById('resetBtn').addEventListener('click',()=>{if(confirm('현재 내용을 초기화할까요?')){data=structuredClone(initial);saveDraft();renderEditor();renderPreview()}});
